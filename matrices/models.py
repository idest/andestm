...
class ThermalInput(models.Model):
    ...
    """
    K_z = models.BooleanField("K = f(z)", help_text="¿Usar K en función de la profundidad?")
    H_z = models.BooleanField("H = f(z)", help_text="¿Usar H en función de la profundidad?")
    EF_lat = models.BooleanField("EF = f(lat)", help_text="¿Usar EF en función de la latitud?")
    EF = models.FloatField("EF", help_text="Edad de la corteza oceánica en la fosa (Ma)")
    K_cs = models.FloatField("K (cs)", help_text="Conductividad térmica corteza sup. (W/mK)")
    K_ci = models.FloatField("K (ci)", help_text="Conductividad térmica corteza inf. (W/mK)")
    K_ml = models.FloatField("K (ml)", help_text="Conductividad térmica manto litosférico (W/mK)")
    H_cs = models.FloatField("H (cs)", help_text="Calor radiogénico corteza sup. (W/m3)")
    H_ci = models.FloatField("H (ci)", help_text="Calor radiogénico corteza inf. (W/m3)")
    H_ml = models.FloatField("H (ml)", help_text="Calor radiogénico manto litosférico (W/m3)")
    kpa = models.FloatField("kpa", help_text="Difusividad térmica (m2/s)")
    T_p = models.FloatField("Tp", help_text="Temperatura potencial del manto (C)")
    G_a = models.FloatField("Ga", help_text="Gradiente adiabático (K/m)")
    V = models.FloatField("V", help_text="Velocidad de convergencia (m/Ma)")
    b = models.FloatField("b", help_text="Parametro adimensional")
    dip = models.FloatField("dip", help_text="Ángulo de subduccion")
    D2 = models.FloatField("D2", help_text="Constante de proporcionalidad adimensional")
    d_rad = models.FloatField("Drad", help_text="Decaimiento radiogénico (km)")
...
class MechanicalInput(models.Model):
    ...
    Cs = models.ForeignKey('RheologicModel', on_delete=models.CASCADE,
        ...
        help_text="Modelo reológico para la corteza sup.")
    Ci = models.ForeignKey('RheologicModel', on_delete=models.CASCADE,
        related_name='ci', verbose_name="C.I.",
        help_text="Modelo reológico para la corteza inf.")
